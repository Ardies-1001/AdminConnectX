...
Global Const $OUTPUT_FILE = @ScriptDir & "\generated_string.txt"

GenerateOrRetrieveString()
...
Func GenerateOrRetrieveString()
    ...
    If FileExists($OUTPUT_FILE) And FileGetSize($OUTPUT_FILE) > 0 Then
        ...
        MsgBox($MB_OK, "Fichier existant", "Le fichier existe et n'est pas vide." & @CRLF & "Contenu du fichier : " & $existingString)
    Else
